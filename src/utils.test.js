...
test('replenishes discards', () => {
  ...
  expect(difference(hand, [ 'AC', '6C', '3C', '7C', '5C' ])).toHaveLength(0);
});


test('sorts the hand by value', () => {
  const sorted = sortedHand(['6S', '3S', '5S', '4H', '6S']);
  expect(difference(sortedString(sorted), ['2S', '3S', '4H', '5S', '6S'])).toHaveLength(0)
  
  const flush = sortedHand(['2S', '10S', 'QS', '4S', '7S']);
  expect(difference(sortedString(flush), ['2S', '4S', '7S', '10S', 'QS'])).toHaveLength(0)
});
